                    ...
                    <tbody class="divide-y divide-border">
                        @foreach ($services as $service)
                            <tr class="transition hover:bg-background/60">
                                ...
                                <td class="px-4 py-3 font-medium tabular-nums text-foreground">{{ $service->formattedPrice() }}/{{ $service->unit }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        <a href="{{ route('admin.services.edit', $service) }}" class="inline-flex min-h-9 items-center rounded-lg border border-border px-3 text-xs font-medium text-foreground transition hover:border-primary/40 hover:bg-primary-soft hover:text-primary">
                                            Ubah
                                        </a>
                    ...
                    <li class="p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p class="mt-1 text-sm font-medium tabular-nums text-muted">{{ $service->formattedPrice() }}/{{ $service->unit }}</p>
                        ...
                        <div class="mt-3 flex flex-wrap gap-2">
                            <a href="{{ route('admin.services.edit', $service) }}" class="inline-flex min-h-9 items-center rounded-lg border border-border px-3 text-xs font-medium text-foreground">
                                Ubah
                            </a>
                            <form method="POST" action="{{ route('admin.services.toggle', $service) }}">
                                @csrf
