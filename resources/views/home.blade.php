            ...
            <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($services as $service)
                    <a href="{{ route('services.show', $service) }}" class="group flex flex-col rounded-2xl border border-border bg-surface p-5 transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-sm">
                        ...
                        <p class="mt-2 line-clamp-2 flex-1 text-sm leading-relaxed text-muted">{{ $service->description }}</p>
        ...
        <div class="mx-auto grid max-w-6xl gap-6 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:px-8">
            <div>
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="{{ route('kontak') }}" class="inline-flex min-h-12 items-center rounded-lg bg-primary px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-dark">
                        Buka halaman kontak
                    </a>
                    @if ($whatsappUrl)
